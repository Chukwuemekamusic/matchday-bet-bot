...
export const createVerifyHandler = (
  ...
  return async (handler, { channelId, userId, eventId, threadId }) => {
    ...
    try {
      ...
      const walletAddresses = await getLinkedWallets(
        context.bot,
        userId as `0x${string}`
      );
      ...
      console.log(`[/verify] User wallets: ${walletAddresses.join(", ")}`);
      ...
      for (const walletAddress of walletAddresses) {
        ...
        const batchResults = await Promise.all(
          walletAddresses.map(async (walletAddress) => {
            return await context.contractService.getBatchUserBets(
              onChainMatchIds,
              walletAddress
            );
          })
        ...
        console.log(
          `[/verify] Verification complete for ${userId} and wallets ${walletAddresses.join(
            ", "
          )}`
        );
      }
